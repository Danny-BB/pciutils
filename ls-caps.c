...
static void cap_express_dev(struct device *d, int where, int type)
{
  ...
  /* Show MPS warning if current MPS is less than capability */
  {
    int devcap_mps = 128 << (t & PCI_EXP_DEVCAP_PAYLOAD);
    int devctl_mps = 128 << ((w & PCI_EXP_DEVCTL_PAYLOAD) >> 5);
    if (devctl_mps < devcap_mps)
      printf("\t\t\t*** Warning: MPS limited to %d bytes (device capable of %d bytes)\n",
             devctl_mps, devcap_mps);
  }
